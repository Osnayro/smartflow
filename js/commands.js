...
// ============================================================
// MÓDULO 4: SMARTFLOW COMMANDS (Parser de Comandos) - v4.6
...
//            accesibilidad, gestión de puertos lógicos, y nuevos comandos INFO.
// ============================================================
...
const SmartFlowCommands = (function() {
    ...
    function parseInfo(cmd) {
        ...
        if (!tag) {
            notifyWithVoice(`Especifique el tag del ${type}`, true);
            return true;
        }
        ...
        if (type === 'line' || type === 'línea' || type === 'linea') {
            ...
            return infoEquipment(tag);
        } else if (type === 'component' || type === 'componente') {
            return infoComponent(tag);
        } else {
    ...
    function infoComponent(tag) {
        ...
        if (!foundComp) {
            notifyWithVoice(`Componente ${tag} no encontrado en ninguna línea`, true);
            return true;
        }
    ...
    function parseConnect(cmd) {
        ...
        if (!nzFrom || !nzTo) { notifyWithVoice("Puerto no encontrado", true); return true; }
        ...
        notifyWithVoice(`Conectado ${fromEquip}.${fromNozzle} con ${toEquip}.${toNozzle} (${tag})`, false, { line: nuevaLinea });
        _renderUI();
        return true;
    }

    // -------------------- 3. PARSER DE RUTA AUTOMÁTICA (MEJORADO: PUERTO DESTINO OPCIONAL) --------------------
    ...
    function parseHelp(cmd) {
        ...
        ayuda += "INFORMACIÓN:\n  info line/línea [TAG]              Muestra tipo, material, coordenadas, conexiones\n  info equipment/equipo [TAG]        Muestra tipo, material, posición, puertos\n  info component/componente [TAG]    Muestra tipo, descripción, línea contenedora\n\n";
    ...
    function executeCommand(cmd) {
        ...
        if (parseInfo(trimmed)) return true; // <-- NUEVO
