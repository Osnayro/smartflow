...
(function() {
    ...
    function scheduleRender() {
        ...
        else if (m === 'iso' && window.currentViewMode === '2d' && window.SmartFlowRenderer) SmartFlowRenderer.render();
        else if (m === 'iso' && window.currentViewMode === '3d' && window.SmartFlowRender) SmartFlowRender.renderFrame();
    }
    ...
    function autoCenter() {
        ...
        else if (m === 'iso' && window.currentViewMode === '2d' && window.SmartFlowRenderer) SmartFlowRenderer.autoCenter();
        else if (m === 'iso' && window.currentViewMode === '3d' && window.ThreeJsEngine) ThreeJsEngine.fitCameraToEquipments();
    }
    ...
    function toggleAllPanels() { 
        var panels = [sidePanel, document.getElementById('toolsPanel')]; 
        var visible = sidePanel && sidePanel.style.display !== 'none'; 
    ...
    function updatePropertyPanel(info) {
        ...
        // Construir panel de propiedades según tipo de objeto
        let html = '';
        html += '<div class="prop-group"><span class="prop-label">TAG</span><span class="prop-value">' + (info.tag || 'N/A') + '</span></div>';
        ...
        if (info.range) {
            html += '<div class="prop-group"><span class="prop-label">RANGO</span><span class="prop-value">' + info.range + '</span></div>';
        }
        if (info.loopTag) {
            html += '<div class="prop-group"><span class="prop-label">LAZO</span><span class="prop-value">' + info.loopTag + '</span></div>';
        }
        if (info.location) {
            html += '<div class="prop-group"><span class="prop-label">UBICACIÓN</span><span class="prop-value">' + info.location + '</span></div>';
        }
        ...
        html += '<div class="prop-group"><span class="prop-label">PUERTOS</span>' + 
            (info.puertos && info.puertos.length ? 
                info.puertos.map(function(p) { 
                    return '<div class="port-item"><span>' + p.id + '</span><span class="' + 
                        (p.status === 'open' ? 'port-open' : 'port-connected') + '">' + 
                        (p.status === 'open' ? 'DISPONIBLE' : 'CONECTADO') + '</span></div>'; 
                }).join('') : '<p>Sin puertos</p>') + '</div>';
    ...
    function initModules() {
        ...
        // 9. Deliverables (PDFs normativos) ★ NUEVO
        if (typeof SmartFlowDeliverables !== 'undefined') {
            ...
            console.log('✅ Deliverables v1.0');
        }
        
        // 10. Despachador de Comandos (SIEMPRE al final)
        ...
        console.log('✅ SmartFlow Pro v5.0 listo | PFD + DTI + ISO + Deliverables');
        notify('SmartFlow Pro v5.0 listo | PFD + DTI + ISO + PDF', false);
    }
    
    // ================================================================
    //  SWITCH DE MÓDULOS (PFD / DTI / ISO)
    // ================================================================
    ...
    function iniciarNuevoProyecto() { 
        ...
        // Actualizar metadatos de deliverables
        if (typeof SmartFlowDeliverables !== 'undefined') {
    ...
    function setTool(mode) { 
        toolMode = mode; 
        var ids = { select: 'toolSelect', moveEq: 'toolMoveEq', editPipe: 'toolEditPipe', addPoint: 'toolAddPoint' }; 
    ...
    function toggleVoice() { 
        ...
        if (b) b.textContent = voiceEnabled ? '🔊 Voz ON' : '🔇 Voz OFF'; 
    }
    
    // ================================================================
    //  ATAJOS DE TECLADO (Ampliados v5.0)
    // ================================================================
    ...
    function ejecutarComando() {
        ...
        if (lineas.length === 1) {
            ok = SmartFlowCommands.executeCommand(lineas[0]) !== false;
        } else {
            ok = SmartFlowCommands.executeBatch(lineas.join('\n')) > 0;
        }
        ...
        if (!['info','list','help','ayuda','validate','validar','summary','resumen','balance'].some(function(k) { 
    ...
    // ================================================================
    //  BINDING DE EVENTOS (Ampliado v5.0)
    // ================================================================
    function bindEvents() {
        ...
        v('btnSetElev', function() { 
            var val = parseInt(customElev ? customElev.value : 0); 
        ...
        window.addEventListener('resize', function() { 
            ...
            rt = setTimeout(function() { 
                ...
                } else if (m === 'iso' && typeof SmartFlowRenderer !== 'undefined') { 
                    SmartFlowRenderer.resizeCanvas(); 
                } 
            }, 150); 
        });
    ...
    function init() {
        ...
        var iv = setInterval(function() { 
            if (mi < msgs.length && ss) { 
                ss.textContent = msgs[mi]; 
                mi++; 
            } 
        }, 600);
